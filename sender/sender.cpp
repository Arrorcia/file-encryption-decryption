...
int sendFile(FILE* fp,unsigned long fsize,unsigned char *path,unsigned char *data_to_encrypt,unsigned char *data_after_encrypt,AES_KEY *AESEncryptKey,int clnt_sock){
    ...
    cout<<endl;
    printf("%d:%s",strlen((char*)sha256),sha256);

    printf("send sha256\n");
    
    AES_encrypt(&sha256[0], &e_sha256[0], AESEncryptKey);
    sendData(&e_sha256[0], 16 ,clnt_sock);
    printf("%s\n",(char *)&e_sha256[0]);

    AES_encrypt(&sha256[16], &e_sha256[16], AESEncryptKey);
    sendData(&e_sha256[16], 16 ,clnt_sock);

    AES_encrypt(&sha256[32], &e_sha256[32], AESEncryptKey);
    sendData(&e_sha256[32], 16 ,clnt_sock);

    AES_encrypt(&sha256[48], &e_sha256[48], AESEncryptKey);
    sendData(&e_sha256[48], 16 ,clnt_sock);


    
    printf("Completes!\n%d\n",strlen((char*)e_sha256));
    return 0;
}
